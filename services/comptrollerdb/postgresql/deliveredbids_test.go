...
func TestDeliveredBids(t *testing.T) {
	...
	originalBids, err := s.DeliveredBids(ctx, &comptrollerdb.DeliveredBidFilter{})
	require.NoError(t, err)
	...
	bid1 := &comptrollerdb.DeliveredBid{
		Slot:                 12345,
		Relay:                "Relay 1",
	...
	require.NoError(t, s.SetDeliveredBid(ctx, bid1))

	bid2 := &comptrollerdb.DeliveredBid{
		Slot:                 12346,
		Relay:                "Relay 2",
	...
	require.NoError(t, s.SetDeliveredBid(ctx, bid2))

	bid3 := &comptrollerdb.DeliveredBid{
		Slot:                 12347,
		Relay:                "Relay 1",
	...
	require.NoError(t, s.SetDeliveredBid(ctx, bid3))

	// Fetch all bids.
	bids, err := s.DeliveredBids(ctx, &comptrollerdb.DeliveredBidFilter{})
	require.NoError(t, err)
