...
func (s *Service) LatestReceivedBidSlot(ctx context.Context) (phase0.Slot, error) {
	ctx, span := otel.Tracer("wealdtech.comptrollerd.services.comptrollerdb.postgresql").Start(ctx, "LatestReceivedBidSlot")
	defer span.End()
	...
	err = tx.QueryRow(ctx, `
SELECT MAX(f_slot)
FROM t_received_bids`).Scan(&latest)
	if err != nil {
		return 0, err
	}
...
func (s *Service) ReceivedBids(ctx context.Context,
	filter *comptrollerdb.ReceivedBidFilter,
) (
	[]*comptrollerdb.ReceivedBid,
	error,
) {
	tx := s.tx(ctx)
	if tx == nil {
